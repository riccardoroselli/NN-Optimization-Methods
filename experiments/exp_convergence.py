...
Runs all A1/A2 variants plus Polyak-optimal A1 and decreasing-mu A2, fits
numerical convergence rates, and produces the full set of convergence plots.

Outputs:
  results/plots/convergence_vs_iter.pdf
  results/plots/convergence_vs_time.pdf
  results/plots/convergence_rates.pdf
  results/plots/convergence_dynamic_mu.pdf
  results/plots/convergence_polyak.pdf
  results/plots/convergence_sparsity_low_lambda.pdf
  results/plots/convergence_sparsity_high_lambda.pdf
  results/logs/convergence_rates.csv
  results/logs/convergence_<variant>.csv  (one per variant)
"""
...
def plot_all_variants(results, f_ref, save_dir):
    """Plot 1: all variants on one semi-log plot (gap vs iteration)."""
    ...
    ax.set_title('Convergence: All Variants')
    ax.legend(fontsize=7, ncol=3, loc='upper right')
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'{save_dir}/convergence_vs_iter.pdf')
    plt.close()
    print(f"  Saved convergence_vs_iter.pdf")


def plot_all_variants_vs_time(results, f_ref, save_dir):
    """Plot 1b: all variants, gap vs wall-clock time."""
    fig, ax = plt.subplots(figsize=(12, 7))
    for i, (name, hist) in enumerate(results.items()):
        gaps = _gap_series(hist, f_ref)
    ...
    ax.set_ylabel(r'$f(w_k) - f^*$')
    ax.set_title('Convergence: All Variants (vs Time)')
    ax.legend(fontsize=7, ncol=3, loc='upper right')
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'{save_dir}/convergence_vs_time.pdf')
    plt.close()
    print(f"  Saved convergence_vs_time.pdf")


def plot_sparsity_low_lambda(results, save_dir, lam=0.01):
    """Plot: sparsity vs iteration at low lambda, reusing the main runs."""
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'{save_dir}/convergence_sparsity_low_lambda.pdf')
    plt.close()
    print(f"  Saved convergence_sparsity_low_lambda.pdf")


def save_per_variant_logs(results, f_ref, save_dir):
    """Save per-variant convergence data as CSV (one file per variant)."""
    for name, hist in results.items():
    ...
    print(f"  Per-variant CSV logs saved to {save_dir}/")
...
def plot_rates(df_rates, obj, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/convergence_rates.pdf')
    plt.close()
    print(f"  Saved convergence_rates.pdf")
...
def plot_dynamic_mu(results, f_ref, save_dir):
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'{save_dir}/convergence_dynamic_mu.pdf')
    plt.close()
    print(f"  Saved convergence_dynamic_mu.pdf")
...
def plot_polyak(results, f_ref, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/convergence_polyak.pdf')
    plt.close()
    print(f"  Saved convergence_polyak.pdf")
...
def plot_sparsity_high_lambda(save_dir, n=2000, d=50, m=200, lam=5.0,
    ...
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f'{save_dir}/convergence_sparsity_high_lambda.pdf')
    plt.close()
    print(f"  Saved convergence_sparsity_high_lambda.pdf")
...
def run_all():
    ...
    df_rates.to_csv(f'{log_dir}/convergence_rates.csv', index=False)
    print(f"\n  Rates saved to {log_dir}/convergence_rates.csv")
    print("\n" + "=" * 90)
    ...
    plot_all_variants_vs_time(results, f_ref, plot_dir)
    plot_rates(df_rates, obj, plot_dir)
    plot_dynamic_mu(results, f_ref, plot_dir)
    plot_polyak(results, f_ref, plot_dir)
    plot_sparsity_low_lambda(results, plot_dir)
    plot_sparsity_high_lambda(plot_dir)

    # Part D: per-variant CSV logs
    save_per_variant_logs(results, f_ref, log_dir)
